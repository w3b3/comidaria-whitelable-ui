...
const fetchMenuItems = async (restaurant_name: string) => {
  const { data } = await axios.get(`https://api.emitajuba.com.br/data/${restaurant_name}`);
...
export const useMenuItems = (restaurant_name: string) => {
  return useQuery({ queryKey: ['menuItems', restaurant_name], queryFn: () => fetchMenuItems(restaurant_name) });
};